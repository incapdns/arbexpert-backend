...
use crate::exchange::gate::GateExchange;
use crate::exchange::gate::public::sub_client::GateSubClient;
use std::error::Error;
use std::rc::Rc;
...
impl GateExchange {
  pub async fn watch_orderbook(&self, symbol: String) -> Result<SharedBook, Box<dyn Error>> {
    ...
    if let Some(client) = {
      let guard = sub_clients.borrow();
      guard.iter().find(|c| c.subscribed_count() < 50).cloned()
    } {
      return client.watch(symbol).await;
    }

    // 3) Caso contrário, cria um novo client
    let new_sc = GateSubClient::new(self.utils.clone(), market, self.public.time_offset_ms);
